...
import { ButtonsContainer, Chat, ChatRow, ChatsContainer, Container, DeleteButton } from './style'
...
export default function Menu ({ onDeleteChat, scrollRef } : MenuProps) {
  ...
  function handleDeleteChat (chatIndex : number) {
    ...
    if (!confirmed) return
    deleteSingleChat(chatIndex)
  }

  function handleSaveChat () {
    const chatIndex = getActiveChatIndex()
    ...
    <Container $isMobile={isMobile}>
      <ChatsContainer>
        { chats.map((messages, index) => {
          ...
          return (
            <ChatRow className='chat-row' key={chatNumber}>
              <Chat to={`${ROUTES.CHAT}/${chatNumber}`} title={content}>
                { content }
              </Chat>
              <DeleteButton
        ...
        { chats.length > 0 && (
          <Chat key='createChat' to={`${ROUTES.CHAT}/${chats.length + 1}`}
